  ...
  <DefaultField :field="field" :errors="errors">
    <template #field>
      <div class="form-group mb-3">
        ...
        >
          {{ field.title_format.replace(':text', value.title || '') }}
        </p>
      </div>
      <div class="form-group mb-3">
      ...
      <div class="form-group mb-3" ref="canonicalLinksWrapper">
        ...
        >
          <input
            ...
            @change="setHasChanged"
          />
          <DefaultButton @click.prevent="removeCanonicalLink(index)"
            >-</DefaultButton
          >
        </div>
...
export default {
  ...
  data() {
    ...
    return {
      hasChanged: false,
      imageFile: null,
      canonicalLinks: field.canonical_links
        ? field.canonical_links.map((link) => {
            return {
              value: link,
            };
          })
  ...
  methods: {
    addCanonicalLink() {
      ...
      this.$nextTick(() => {
        const inputs =
          this.$refs.canonicalLinksWrapper.querySelectorAll('input');
    ...
    fill(formData) {
      ...
      this.value.params.canonical_links = this.canonicalLinks.map(
        (link) => link.value
      );
